...
def _after_assignment(source: str, target: str, function: str | None = None):
    lines = source.splitlines(True)
    hits = []
    tree = ast.parse(source)
    if function:
        scopes = [n for n in ast.walk(tree) if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == function]
        if len(scopes) != 1:
...
def _before(source: str, anchor: str):
    lines = source.splitlines(True)
...
class ModularPatcher:
    ...
    def __init__(self, run_path: Path):
        ...
        self.names = ('run_inbound.py', 'run_turn.py', 'run_turn_runner.py', 'run_busy.py')
    ...
    def _plan(self):
        ...
        add('run_turn.py', lambda s: _before(s, 'return await self._hmwa_deliver_turn_response('),
        ...
        add('run_busy.py', lambda s: p._find_stop_site(ast.parse(s), s.splitlines(True)), p._stop_hook)
        ...
        add('run_turn.py', lambda s: _before(s, 'if not result.get("interrupted"):'),
            lambda i: p._followup_complete_hook(i).replace('message_id=event_message_id', 'message_id=turn_ctx.event_message_id'))
    ...
    def _publish(self, before, after):
        written = []
        try:
...
class ModularCronPatcher(ModularPatcher):
    MARKERS = [(p.MK_CRON_DELIVER, p.MK_CRON_DELIVER_END)]

    def __init__(self, cron_path):
    ...
    def _plan(self):
        ...
        hook = p._make_hook(indent, p.MK_CRON_DELIVER, p.MK_CRON_DELIVER_END, [
            'try:',
            "    if (t.platform_name.lower() in ('feishu', 'lark')",
            "            and not getattr(t.transport, 'is_relay', False)",
            '            and not media_files and not t.in_channel_surface and not t.thread_id):',
            ...
            "                           loop=loop, task_name=job.get('name', ''), run_time=job.get('next_run_at', '')):",
