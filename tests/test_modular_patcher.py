...
from hermes_lark_streaming.patcher import CronPatcher, Patcher, PatcherError
...
def test_roundtrip_and_idempotence(modular_root, kind):
    obj = (
        Patcher(modular_root / "gateway/run.py")
        if kind == "gateway"
        else CronPatcher(modular_root / "cron/scheduler.py")
    )
...
def test_modular_commentary_keeps_tts_segment_boundaries(modular_root, already_streamed):
    ...
    namespace = {
        "ctx": SimpleNamespace(event_message_id="m", _run_still_current=lambda: True),
        "stts": stts,
        "stream_consumer": native,
    }
    exec(
        compile(ast.fix_missing_locations(ast.Module(body=[callback], type_ignores=[])), "<commentary>", "exec"),
        namespace,
    )
