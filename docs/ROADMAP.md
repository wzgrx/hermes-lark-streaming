...
1. **原生插件钩子替代 AST**：跟进 [Hermes CardKit 整合讨论](https://github.com/NousResearch/hermes-agent/issues/33854)，当官方暴露 answer/reasoning/tool/approval 生命周期后删除对 `gateway/run_*` 的源码注入。
...
9. **`lark-cli` smoke chain**：使用 [larksuite/cli](https://github.com/larksuite/cli) 做权限/接口 dry-run 和回调事件预检；它是运维/验收工具，不成为插件运行时依赖。
